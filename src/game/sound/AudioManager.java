...
public class AudioManager {
	...
	{
		FileHandle[] soundFiles = Gdx.files.local("res/sounds/").list();

		for(int i = 0; i < soundFiles.length; i++)
		{
			FileHandle handle = soundFiles[i];
			Audio audio = new Sound(handle);
			add(audio, handle.nameWithoutExtension().toLowerCase());
		}
	}
